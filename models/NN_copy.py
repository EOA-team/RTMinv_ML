...
class NeuralNetworkRegressor2(nn.Module):
  ...
  optim={'name': 'Adam', 'learning_rate': 0.01}, criterion='MSE', random_state=42):
      ...
      :param random_state: 
      '''
      super(NeuralNetworkRegressor2, self).__init__()
  ...
  def fit(self, X: np.array, y: np.array, X_val: np.array, y_val: np.array):
      '''
      ...
      for epoch in range(self.epochs):
          ...
          # Check validation set performance and early stopping
          self.model.eval()  # Set model to evaluation mode
          with torch.no_grad():  # Disable gradient calculation for validation
              val_outputs = self.model(X_val)
              val_loss = self.criterion(val_outputs, y_val.float())

          print(f'Epoch {epoch} Val loss {val_loss} patience {patience}')
          if val_loss < best_loss:
              best_loss = val_loss    
              patience = 80  # Reset patience counter
          else:
              patience -= 1
              if patience == 0:
                  break
